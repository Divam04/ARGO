...
> - No API keys or credentials in the client. Vertex AI and the SMTP relay are
>   called only from Cloud Functions. If you find yourself putting a key,
>   password, or SMTP host string in Dart, stop.
...
1. **The agent puts an API key or the SMTP password in the Flutter app**
   because it is the shortest path to a working demo. Check the diff every
   phase — an SMTP password in the client is worse than a leaked API key,
   since it lets anyone who unzips the APK send arbitrary email as your
   domain, not just call your Gemini quota.
...
5. **`bounced` never populates.** Plain SMTP gives a synchronous accept/reject
   at send time and nothing else — no async bounce webhook the way a
   transactional email API would give you. If the Email Log's bounced filter
   (§5.16) is meant to mean something, the SMTP relay has to be paired with
   its own delivery-webhook mechanism (see spec §9.6/A7). Otherwise every
   email only ever reads `sent` or `failed`, and a bounced PIN email looks
   identical to a delivered one — which defeats the entire reason §5.16 calls
   bounces out by name.
